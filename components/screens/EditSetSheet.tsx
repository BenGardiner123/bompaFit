...
/**
 * The form's working copy. `startWeight` is the weight as first shown, kept so
 * Save can tell whether the weight was actually touched.
 */
type Draft = { id: number; weight: number; startWeight: number; reps: number; rpe: number; type: SetType };
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {row.rpeEstimated && draft.rpe === row.rpe && (
          <span style={{ fontSize: 12.5, fontWeight: 600, lineHeight: 1.45, color: onInk.body }}>
      ...
      {piece ? (
        ...
        <span style={{ fontSize: 12.5, fontWeight: 600, lineHeight: 1.45, color: onInk.body, ...num }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <InkButton width={100} height={56} fontSize={14} color={C.redLight} onClick={remove}>
          Delete
        </InkButton>
        <InkButton variant="amber" height={56} fontSize={16} onClick={save} style={{ flex: 1 }}>
          Save changes
        </InkButton>
      </div>
    </DarkSheet>
